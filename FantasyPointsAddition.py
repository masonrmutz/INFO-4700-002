...
import altair as alt  # still here if you want to use it later
...
def add_fantasy_points(df: pd.DataFrame, stat_type: str) -> pd.DataFrame:
    """
    Adds FantasyPts and FantasyPtsPerGame to the dataframe based on stat_type.
    Scoring:
    ...
    if stat_type == "QB Passing":
        ...
        # Rushing / Receiving: 1 pt per 10 yards, 6 per TD, 1 per REC (PPR)
        yds = df["YDS"] if "YDS" in df.columns else 0
        tds = df["TD"] if "TD" in df.columns else 0
...
app_ui = ui.page_navbar(
    ...
    ui.nav_panel("Interactive Chart", ui.output_plot("stat_plot")),  # chart tab wired to stat_plot
